...
{  
    ...
    {  
        ...
        if ((fileName.size() >= 4 && strcmp(fileName.c_str() + fileName.size() - 4, ".jpg") == 0) ||  
            ...
            ret = inference_yolov12_model(rknn_app_ctx, &src_image, &od_results);  
            if (ret != 0) {  
                printf("inference_yolov12_model fail! ret=%d\n", ret);  
...
{   
    ...
    const std::string modelPath = "/data/github_project/YOLOv12_RK3588_object_detect/model/26119best.rknn";  
    const std::string imageFolder = "/data/github_project/YOLOv12_RK3588_object_detect/inputimage";  
    const std::string outputFolder = "/data/github_project/YOLOv12_RK3588_object_detect/outputimage"; 
    ...
    ret = init_yolov12_model(modelPath.c_str(), &rknn_app_ctx);  
    if (ret != 0) 
    {  
        printf("init_yolov12_model fail! ret=%d model_path=%s\n", ret, modelPath.c_str());  
        return -1;  
    }      

    processImagesInFolder(imageFolder, &rknn_app_ctx, outputFolder); 

    ret = release_yolov12_model(&rknn_app_ctx); 

    if (ret != 0) 
    {  
        printf("release_yolov12_model fail! ret=%d\n", ret);  
    }  
